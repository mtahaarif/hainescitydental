...
const TARGET_TITLE = 'Generous gift made to Wounded Warrior Project by our patient on\n behalf of Haines City Dental';
...
// find item (some titles include newlines in JSON; normalize)
const item = items.find(i => ((i.title||'').replace(/\s+/g,' ').trim()) === (TARGET_TITLE.replace(/\s+/g,' ').trim()));
if (!item) { console.error('Item not found:', TARGET_TITLE); process.exit(2); }

// parse item's html to extract all image srcs
